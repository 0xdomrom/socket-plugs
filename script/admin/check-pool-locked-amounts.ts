    ...
    for (const chain of Object.keys(addresses)) {
      ...
      for (const token of Object.keys(addresses[chain])) {
        ...
        for (const connectorChain of Object.keys(
          ...
          const connector =
            addresses[chain][token].connectors[connectorChain]["FAST"];

          const poolId = await hookContract.functions.connectorPoolIds(
            connector
          );
          const limit = await hookContract.functions.poolLockedAmounts(
            poolId[0]
          );

          const limit_f = +ethers.utils.formatUnits(
            limit[0],
            tokenDecimals[token]
          );
          ...
          const tokenAddress =
            addresses[connectorChain][token].NonMintableToken;
          ...
          const balance_f = +ethers.utils.formatUnits(
            vaultBalance,
            tokenDecimals[token]
          );
          ...
          if (Math.abs(limit_f - balance_f) > 0.01) {
            console.log(
              `Mismatching balances for connector ${connectorChain} token ${token} poolId ${poolId[0]}`
            );

            toUpdate.push([poolId[0], vaultBalance]);
          }
        }

        if (toUpdate.length > 0) {
          console.log(
            `// ${chain}\n_call(${hookContract.address}, hex"${(
              await hookContract.populateTransaction["updatePoolLockedAmounts"](
                toUpdate.map(([x, _]) => x),
                toUpdate.map(([_, x]) => x)
              )
            ).data.slice(2)}");`
